...
export default function ClientAboutView({ data }) {
  ...
  return (
    <div
      className="flex flex-col inner justify-center items-center h-screen pb-20 sm:mt-14 sm:mb-14 px-6 sm:px-8 lg:px-16 mx-auto"
      id="about"
    >
          ...
          >
            {data?.skills.split(",").map((skill) => (
              <motion.div className="w-full flex justify-center items-center">
                <button className="whitespace-nowrap text-ellipsis overflow-hidden py-3 w-[160px] px-6 border-[2px] border-red-main bg-[#fff] text-[#000] font-semibold rounded-lg text-xl tracking-widest hover:shadow-red-main transition-all outline-none ">
                  {skill}
                </button>
              </motion.div>
            ))}
